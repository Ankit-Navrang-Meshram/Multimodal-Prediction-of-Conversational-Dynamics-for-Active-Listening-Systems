...
parser.add_argument("--fusion_module" , type=str, default=None)
# data params
...
def compute_loss(criterion, pred, y, model):
    """
    Computes cross-entropy loss + optional HMEF auxiliary routing loss.
    Works with all fusion modules (aux_loss is 0.0 for non-HMEF modules).
    """
...
def init_log_dir():
    ...
    task_dir = os.path.join(args.log_dir, f"{time_str}{args.model_name}{args.fusion_module}")
...
def main():
    ...
    for p in model.audio_model.parameters(): p.requires_grad = False
    ...
    for epoch in t_bar:
        ...
        for i, (text, audio, vision, y) in enumerate(train_loader):
            ...
            #loss = criterion(pred, y)
            loss = compute_loss(criterion, pred, y, model)
            loss.backward()
            optimizer.step()
